...
let dump_rec_name name = function
  | Some ty -> dump_name name ^^^ colon ^^^ dump_ty ty
  | None -> dump_name name

let rec dump_let ?rec_ty (name, t) =
  str "let" ^^^ dump_rec_name name rec_ty ^^^ equals ^//^ dump_t t
...
let dump_lets values =
  separate_map (hardline ^^ hardline) (dump_let ?rec_ty:None) values
